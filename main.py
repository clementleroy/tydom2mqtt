...
try:
    with open('/data/options.json') as f:
        ...
        try:
            ...
            if data['TYDOM_IP'] != '':
                TYDOM_IP = data[
                    'TYDOM_IP']  # , 'mediation.tydom.com') # Local ip address, default to mediation.tydom.com for remote connexion if not specified
...
hassio = MQTT_Hassio(broker_host=MQTT_HOST, port=MQTT_PORT, user=MQTT_USER, password=MQTT_PASSWORD, mqtt_ssl=MQTT_SSL,
                     home_zone=TYDOM_ALARM_HOME_ZONE, night_zone=TYDOM_ALARM_NIGHT_ZONE, tydom=tydom_client)
    ...
    while True:
        ...
        try:
            ...
            while True:
                # listener loop
                try:
                    incoming_bytes_str = await asyncio.wait_for(tydom_client.connection.recv(),
                    ...
                    try:
                        ...
                        print('TimeoutError or websocket error - retrying connection in {} seconds...'.format(
                ...
                handler = TydomMessageHandler(incoming_bytes=incoming_bytes_str, tydom_client=tydom_client,
                                              mqtt_client=hassio)
                try:
